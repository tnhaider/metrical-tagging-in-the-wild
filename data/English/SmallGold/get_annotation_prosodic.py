...
def align_syllables(linetext, footmeter, syllabify='nn'):
	syllable_position = []
	syll_seq = []
	pos_syll = []
	simple_pos_syll = []
	...
	print('DEBUG', len(meter) == len(syll_seq), len(meter), len(syll_seq), meter, get_versification(meter),  syll_seq, pos_syll, syll_position, linetext)
	if len(meter) != len(syll_seq) and syllabify=='nn':# and len(real) != len(syllables):
		pass
		print()
		print('NN', len(meter) == len(syll_seq), len(meter), len(syll_seq), meter, get_versification(meter),  syll_seq, pos_syll, syll_position, linetext)
	if len(meter) != len(syll_seq) and syllabify=='pyphen':# and len(real) != len(syllables):
		pass
		print()
		print('PYH', len(meter) == len(syll_seq), len(meter), len(syll_seq), meter, get_versification(meter),  syll_seq, pos_syll, syll_position, linetext)
	if len(meter) != len(syll_seq):# and len(real) != len(syllables):
		if syllabify == 'nn':
			return align_syllables(linetext, footmeter, syllabify='pyphen')
...
if __name__ == '__main__':
	...
	prosodic_file = open(sys.argv[1]).readlines()
	...
	for poemline in prosodic_file:
		poemline = poemline.strip()
		poemline = poemline.split('\t')
		try:
			linetext = poemline[6]
			linetext = normalize_characters(linetext)
			footmeter = poemline[9]
		except IndexError:
			continue	
		feature_line = align_syllables(linetext, footmeter)
	...
	X_train, X_testval, y_train, y_testval = train_test_split(feature_lines, y, test_size=0.3, random_state=136)
	X_test, X_val, y_test, y_val = train_test_split(X_testval, y_testval, test_size=0.5, random_state=89)
	print('Train Len', len(X_train))
	print('Test Len', len(X_test))
	print('Val Len', len(X_val))
	trainfile = open('prosodic.meter.train.3fold.txt', 'w')
	for f_line in X_train:
		...
		trainfile.write('\n')
	trainfile.close()
	testfile = open('prosodic.meter.test.3fold.txt', 'w')
	for f_line in X_test:
		...
		testfile.write('\n')
	testfile.close()
	valfile = open('prosodic.meter.dev.3fold.txt', 'w')
	for f_line in X_val:
		for i in f_line:
			line = '\t'.join([str(m) for m in i])
			valfile.write(line)
